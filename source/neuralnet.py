...
class SkipGANomaly(object):
    ...
    def __init__(self, height, width, channel, z_dim, w_enc=1, w_con=50, w_adv=1, leaning_rate=1e-3):
        ...
        self.z_code, self.x_hat, self.z_code_hat, self.dis_x, self.dis_x_hat, self.features_real, self.features_fake =\
            self.build_model(input=self.x, ksize=self.k_size,channel=self.channel)
    ...
    def build_model(self, input,channel, ksize=3):

        with tf.name_scope('generator') as scope_gen:
            z_code = self.encoder(input=input, ksize=ksize,channel=channel)
            x_hat = self.decoder(input=z_code, ksize=ksize,channel=channel)
            z_code_hat = self.encoder(input=x_hat, ksize=ksize,channel=channel)

        with tf.name_scope('discriminator') as scope_dis:
            dis_x, features_real = self.discriminator(input=input, ksize=ksize,channel=channel)
            dis_x_hat, features_fake = self.discriminator(input=x_hat, ksize=ksize,channel=channel)

        return z_code, x_hat, z_code_hat, dis_x, dis_x_hat, features_real, features_fake

    def encoder(self, input,channel, ksize=3):

        print("\nEncode-1")
        conv1_1 = self.conv2d(input=input, stride=1, padding='SAME', \
            filter_size=[ksize, ksize, channel, 16], activation="lrelu", name="enconv1_1")
    ...
    def decoder(self, input,channel, ksize=3):
        ...
        convt3_3 = self.conv2d(input=convt3_2, stride=1, padding='SAME', \
            filter_size=[ksize, ksize, 16, channel], activation="None", name="deconv3_3")
        convt3_3 = tf.compat.v1.clip_by_value(convt3_3, 1e-12, 1-(1e-12))

        return convt3_3

    def discriminator(self, input,channel, ksize=3):

        featurebank = []

        print("\nDiscriminate-1")
        conv1_1 = self.conv2d(input=input, stride=1, padding='SAME', \
            filter_size=[ksize, ksize, channel, 16], activation="elu", name="disconv1_1")
